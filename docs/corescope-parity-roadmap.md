...
| Hash stats | Stats Hash tab with size mix, multibyte share, timeline, all-hop collision matrix, inconsistent samples, and prefix lookup | Partial | `/api/v1/stats/hash`; `/api/v1/stats/hash-prefix`; `src/features/stats/HashTab.tsx` | Add deeper node-level collision explainers and command-palette shortcut | Hash explorer polish | P1 |
| Hash collision matrix | All-hop collision matrix grouped by IATA/hash-size with risky-cell prefix lookup | Done | `/api/v1/stats/hash`; `HashTab.tsx` | Maintain browser acceptance and add richer drilldowns if operators need them | Existing hash analytics API | P0 |
